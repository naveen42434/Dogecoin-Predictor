...
from preprocess import transform_ts_data_into_features_and_target, get_preprocessing_pipeline
from hyperparams import find_best_hyperparams
...
df = pd.read_parquet(DATA_DIR + "/" + 'ohlc_data.parquet')
...
def train(
    ...
    model_name = f"{model}_{timestamp}"
    ...
    # log model artifact
    experiment.log_model(model_name, str(model_path))
